...
import { Container, Row, Col, Card, Table, Button, Form, Alert, Modal, Tabs, Tab } from 'react-bootstrap';
...
const AdminPage = () => {
	...
	const handleCreatePet = async (e) => {
		...
		setError('');
		try {
			const created = await createPet(e.currentTarget ? {
				name: e.currentTarget.form ? e.currentTarget.form.name?.value : undefined,
				...newPet
			} : newPet);
						...
						<Row className="g-3">
							<Col md={3}>
								<div className="d-flex flex-column gap-2">
									<Button className={(activeKey === 'dashboard' ? 'btn-orange' : 'btn-cream') + ' btn-wide'} onClick={() => setActiveKey('dashboard')}>Dashboard</Button>
									<Button className={(activeKey === 'pets' ? 'btn-orange' : 'btn-cream') + ' btn-wide'} onClick={() => setActiveKey('pets')}>Pets</Button>
									<Button className={(activeKey === 'tasks' ? 'btn-orange' : 'btn-cream') + ' btn-wide'} onClick={() => setActiveKey('tasks')}>Care Tasks</Button>
									<Button className={(activeKey === 'visits' ? 'btn-orange' : 'btn-cream') + ' btn-wide'} onClick={() => setActiveKey('visits')}>Visits</Button>
									<Button className={(activeKey === 'users' ? 'btn-orange' : 'btn-cream') + ' btn-wide'} onClick={() => setActiveKey('users')}>Users</Button>
									<Button className={(activeKey === 'settings' ? 'btn-orange' : 'btn-cream') + ' btn-wide'} onClick={() => setActiveKey('settings')}>Settings</Button>
								</div>
							</Col>
							<Col md={9}>
								<Tabs activeKey={activeKey} onSelect={(k) => setActiveKey(k || 'pets')} className="mb-3" mountOnEnter unmountOnExit>
