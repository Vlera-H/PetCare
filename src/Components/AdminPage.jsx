...
{activeSection === 'pets' && (
  <>
    <div className="pc-section-title">Manage Pets</div>
    ...
    <Table striped hover responsive className="pets-table">
      ...
      <tbody>
        {pets.map(p => (
          <tr key={p.id}>
            <td>{p.id}</td>
            <td><Form.Control size="sm" value={p.name} onChange={(e) => setPets(prev => prev.map(x => x.id === p.id ? { ...x, name: e.target.value } : x))} /></td>
            <td><Form.Control size="sm" value={p.breed} onChange={(e) => setPets(prev => prev.map(x => x.id === p.id ? { ...x, breed: e.target.value } : x))} /></td>
            <td><Form.Control type="date" size="sm" value={(p.birthDate || '').slice(0,10)} onChange={(e) => setPets(prev => prev.map(x => x.id === p.id ? { ...x, birthDate: e.target.value } : x))} /></td>
...
{activeSection === 'tasks' && (
  <>
    <div className="pc-section-title">Manage Care Tasks</div>
    <Form onSubmit={handleCreateTask} className="mb-2" autoComplete="off">
      <Row className="g-2 align-items-end">
        ...
        <Col md={3}>
          ...
          <Form.Control value={newTask.petId} onChange={(e) => setNewTask(t => ({ ...t, petId: Number(e.target.value) }))} />
        </Col>
        <Col md="auto">
    ...
    <Table striped hover responsive className="pets-table">
      ...
      <tbody>
        {careTasks.map(t => (
          <tr key={t.id}>
            <td>{t.id}</td>
            <td><Form.Control size="sm" value={t.description} onChange={(e) => setCareTasks(prev => prev.map(x => x.id === t.id ? { ...x, description: e.target.value } : x))} /></td>
            <td><Form.Control type="date" size="sm" value={(t.dueDate || '').slice(0,10)} onChange={(e) => setCareTasks(prev => prev.map(x => x.id === t.id ? { ...x, dueDate: e.target.value } : x))} /></td>
            <td><Form.Check type="checkbox" checked={!!t.isCompleted} onChange={(e) => setCareTasks(prev => prev.map(x => x.id === t.id ? { ...x, isCompleted: e.target.checked } : x))} /></td>
            <td><Form.Control size="sm" value={t.petId} onChange={(e) => setCareTasks(prev => prev.map(x => x.id === t.id ? { ...x, petId: Number(e.target.value) } : x))} /></td>
            <td className="d-flex gap-2">
...
{activeSection === 'visits' && (
  <>
    <div className="pc-section-title">Manage Visits</div>
    <Form onSubmit={handleCreateVisit} className="mb-2" autoComplete="off">
      <Row className="g-2 align-items-end">
        <Col md={4}><Form.Label className="fw-semibold">Reason</Form.Label><Form.Control value={newVisit.reason} onChange={(e) => setNewVisit(v => ({ ...v, reason: e.target.value }))} /></Col>
        <Col md={3}><Form.Label className="fw-semibold">Visit Date</Form.Label><Form.Control type="date" value={newVisit.visitDate} onChange={(e) => setNewVisit(v => ({ ...v, visitDate: e.target.value }))} /></Col>
        <Col md={3}><Form.Label className="fw-semibold">Pet ID</Form.Label><Form.Control value={newVisit.petId} onChange={(e) => setNewVisit(v => ({ ...v, petId: Number(e.target.value) }))} /></Col>
        <Col md="auto"><Button className="btn-orange" type="submit" disabled={!newVisit.reason || !newVisit.visitDate || !newVisit.petId}>+ Create</Button></Col>
      </Row>
    </Form>
    <Table striped hover responsive className="pets-table">
      <thead>
      ...
      <tbody>
        {visits.map(v => (
          <tr key={v.id}>
            <td>{v.id}</td>
            <td><Form.Control type="date" size="sm" value={(v.visitDate || '').slice(0,10)} onChange={(e) => setVisits(prev => prev.map(x => x.id === v.id ? { ...x, visitDate: e.target.value } : x))} /></td>
            <td><Form.Control size="sm" value={v.reason} onChange={(e) => setVisits(prev => prev.map(x => x.id === v.id ? { ...x, reason: e.target.value } : x))} /></td>
            <td><Form.Control size="sm" value={v.petId} onChange={(e) => setVisits(prev => prev.map(x => x.id === v.id ? { ...x, petId: Number(e.target.value) } : x))} /></td>
            <td className="d-flex gap-2">
...
{activeSection === 'users' && (
  <>
    <div className="pc-section-title">Manage Users</div>
    ...
    <Table striped hover responsive className="pets-table">
      ...
      <tbody>
        {users.map(u => (
          <tr key={u.id}>
            <td>{u.id}</td>
            <td><Form.Control size="sm" value={u.firstName || ''} onChange={(e) => setUsers(prev => prev.map(x => x.id === u.id ? { ...x, firstName: e.target.value } : x))} /></td>
            <td><Form.Control size="sm" value={u.lastName || ''} onChange={(e) => setUsers(prev => prev.map(x => x.id === u.id ? { ...x, lastName: e.target.value } : x))} /></td>
            <td><Form.Control size="sm" value={u.email || ''} onChange={(e) => setUsers(prev => prev.map(x => x.id === u.id ? { ...x, email: e.target.value } : x))} /></td>
            <td><Form.Select size="sm" value={u.role || 'User'} onChange={(e) => setUsers(prev => prev.map(x => x.id === u.id ? { ...x, role: e.target.value } : x))}><option>User</option><option>Admin</option></Form.Select></td>
            <td className="text-center"><Form.Check type="checkbox" checked={!!(u.isActive ?? false)} onChange={(e) => setUsers(prev => prev.map(x => x.id === u.id ? { ...x, isActive: e.target.checked } : x))} /></td>
            <td className="d-flex gap-2"><Button size="sm" variant="success" onClick={() => handleUpdateUser(u)}>Save</Button><Button size="sm" variant="outline-danger" onClick={() => handleDeleteUser(u.id)}>Delete</Button></td>
          </tr>
        ))}
      </tbody>
    </Table>
